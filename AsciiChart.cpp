	...
	try {
	    ...
	    if(out) {
    		...
    		for (uint8_t c{0}; c < UCHAR_MAX; ++c) {
    			...
    			//        if(c==9 || c==27)
    			out << "<tr><td>" << std::dec << static_cast<char>(c) << "</td><td>" << 
					std::oct << static_cast<int>(c) << "</td><td>" << std::hex << 
					static_cast<int>(c) << "</td><td>" << std::bitset<8>(c) << 
					"</td></tr>" << std::endl;
    			//     else
